...
export async function openModelTts() {
    const { settingsTtsShow } = getSettingsStoreDataTts()
    try {
...
export async function getConfigurationInfo() {
    const { pcInfoTts } = getSettingsStoreDataTts()
    try {
        const res = await post("/manager/get_configuration_info")
        pcInfoTts.value = res.message
    } catch (error) {
        sendLog(error as Error)
    }
}
...
export async function getVisibleModelList() {
    const { visibleModelListTts, settingsTtsShow, managerInstallConfirmTts, isInstalledManagerTts, isResetModelListTts, ollamaUrlTts } = getSettingsStoreDataTts();
    try {
        const res = await post("/manager/get_model_manager")
        if (res.code == 200) {
            isInstalledManagerTts.value = res.message.status
            ollamaUrlTts.value = res.message.ollama_host
            // 如果status为false说明本地没有模型管理器，要求对方安装
            if (res.message.status == false) {
                settingsTtsShow.value = true
                managerInstallConfirmTts.value = true
            } else {
                managerInstallConfirmTts.value = false
            }
            // isInstalledManager.value = true
            visibleModelListTts.value = res.message.models
            if (isResetModelListTts.value.type == 1) {
                isResetModelListTts.value.status = true
            }
        }
    } catch (error) {
        sendLog(error as Error)
...
export async function getModelInstallProgress(callback?: () => void) {
    const { modelInstallProgressTts, installShowTts, modelNameForInstallTts, downloadTextTts, settingsTtsShow } = getSettingsStoreDataTts();
    let timer = setInterval(async () => {
        const res = await post("/manager/get_model_install_progress", modelNameForInstallTts.value)
        if (res.message.status == 3) {
            message.success($t("安装成功"))
            installShowTts.value = false
            downloadTextTts.value = $t("正在连接，请稍候...")
            clearInterval(timer)
        ...
        const modelNameFull = modelNameForInstallTts.value.model + ":" + modelNameForInstallTts.value.parameters
        if (res.message.status == 0) {
            // 等待下载
            downloadTextTts.value = $t("等待下载:{0}，请稍候...", [modelNameFull])
        }

        if (res.message.status == 1) {
            // 正在下载
            downloadTextTts.value = $t("正在下载:{0}，请稍候...", [modelNameFull])
        }

        if (res.message.status == 2) {
            // 正在安装
            downloadTextTts.value = $t("正在安装:{0}，请稍候...", [modelNameFull])
        }
        ...
        modelInstallProgressTts.value = res.message
    }, 1000)
}
...
export async function removeModel() {
    const { modelForDelTts, modelDelLoadingTts, modelDelConfirmTts } = getSettingsStoreDataTts()
    const [model, parameters] = modelForDelTts.value.split(":")
    ...
    modelDelLoadingTts.value = false
    modelDelConfirmTts.value = false
}
