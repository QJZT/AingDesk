<template>
  <div class="live-console" :style="{backgroundColor: themeThinkBg}">
    ...
    <div style="padding: 12px 12px 12px 12px;">
      <n-card title="基础设置" :segmented="{content: true,footer:true}"
    ...
    <!-- 三列布局 -->
     <div class="three-column-layout">
      ...
      >
        ...
        <n-infinite-scroll style="height: 480px;overflow-y: auto;background: var(--n-color-embedded);border-radius: var(--n-border-radius);" :distance="10">
          <div class="">
            <div class="control-block" v-for="(block, index) in modules" :key="index" style="">
              ...
              <div class="block-header">
                ...
                <n-switch v-model:value="block.retAi" size="small" @update:value="handleVolumeChange(block)" v-if="block.module_type == 'base'">
                  <template #checked>AI改写</template>
                  <template #unchecked>AI改写</template>
                </n-switch>
              ...
              <div style="display: flex; align-items: center; gap: 8px; width: 100%;">
                <div style="width: 30%;font-size: 12px;">音量:{{block.volume}}%</div>
                <n-slider
              ...
              <div style="display: flex; align-items: center; gap: 8px; width: 100%;" v-if="block.module_type == 'base'">
                <div style="width: 30%;font-size: 12px">音速:{{block.speed}}x</div>
                <n-slider
      ...
      <!-- 观众互动 -->
      <n-card title="观众互动" :segmented="{content: true,footer:true}"
              header-style="padding:10px;font-size:14px"
              footer-style="padding:10px" content-style="padding:10px;height:100%">
        <template #header-extra>
          <div class="setting-item">
        ...
        <n-divider title-placement="left" style="font-size: 15px;height: 1px;margin: 12px 0;"></n-divider>
...
const { activeKnowledge, activeKnowledgeForChat } = getKnowledgeStoreData();
...
onMounted(() => {
  ...
  socket.value.on('ConnectEvent', function(message) {
    if (!selectedFilters.value.includes("ConnectEvent")) return
  ...
  socket.value.on('CommentEvent', function(message) {//弹幕消息
    ...
    if (!selectedFilters.value.includes("CommentEvent")) return;
  ...
  socket.value.on('GiftEvent', function(message) {//礼物消息
    ...
    if (!selectedFilters.value.includes("GiftEvent")) return;
  ...
  socket.value.on('JoinEvent', function(message) {// 进入直播间
    ...
    if (!selectedFilters.value.includes("JoinEvent")) return
  ...
  socket.value.on('ShareEvent', function(message) { //分享
    if (!selectedFilters.value.includes("ShareEvent")) return;
    console.log("ShareEvent:", message);
    addMessage({content: message.s, data_types: "ShareEvent"});
  });
  socket.value.on('FollowEvent', function(message) {
    if (!selectedFilters.value.includes("FollowEvent")) return;
    console.log("FollowEvent:", message);
    addMessage({content: message.s, data_types: "FollowEvent"});
  });
  socket.value.on('LikeEvent', function(message) {
    EnterSupportRoomUserName.value = message.u;
    if (!selectedFilters.value.includes("LikeEvent")) return;
...
//循环模块处理
const SceneLoop = async (module, newuuid) => {
  let index = 0 //当前播放索引
  ...
  do {
    ...
    if (module.retAi) { //是否改写
      ...
      let prompt = await ReplaceText(promptText.value)
      content = await ReplaceText(content)
...
const includesLike = async (module, newuuid) => {
  let index = 0 //当前播放索引
  const script_content_len = module.script_content.length //脚本长度
  do {
    if (!module.isActive) {
...
const SendGift = async (module, newuuid) => {
  let index = 0//当前播放索引
  const script_content_len = module.script_content.length //脚本长度
  do {
    if (!module.isActive) {
...
//进入直播间 EnterLiveRoom
const includesEnterLiveRoom = async (module, newuuid) => {
  let index = 0 //当前播放索引
  const script_content_len = module.script_content.length//脚本长度
  do {
    if (!module.isActive) {
    ...
    while (EnterLiveRoomUserName.value == "") { //等待有用户送礼物
      await new Promise(resolve => setTimeout(resolve, 1000));
    }
    let content = module.script_content[index];
    if (module.retAi) { //是否改写
...
/**
 * 发送用户消息并生成语音
 * @description 将用户输入的文本转换为语音文件并添加到播放列表
